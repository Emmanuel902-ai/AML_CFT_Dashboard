    ...
    dbc.Tabs([
        ...
        dbc.Tab(label='Performances', tab_id='tab-modeling', children=[
            html.Div([
                ...
                html.P("Explanation of Score Calculation: The scores for Precision, Recall, and F1 Score across Random Forest, Logistic Regression, HDBSCAN, and Isolation Forest were determined using the feature_dataset.csv dataset. Key features like Total_outflow and Sender_diversity underwent preprocessing with one-hot encoding and median imputation, guided by thorough Exploratory Data Analysis (EDA). Each model was trained using cross-validation to ensure reliability, predicting the Is_laundering status (y_pred) and comparing it against the ground truth (y_true) with the classification_report function (zero_division=0). The metrics were calculated as follows: Precision (the ratio of true positives to the sum of true positives and false positives), Recall (the ratio of true positives to the sum of true positives and false negatives), and F1 Score (the harmonic mean of Precision and Recall, computed as 2 * Precision * Recall / (Precision + Recall)). These values were averaged across multiple validation folds, cross-checked with EDA findings for accuracy, and saved in the TRAINING_METRICS dictionary for use in the dashboard.", style={'textAlign': 'center', 'color': '#7f8c8d', 'margin': '10px', 'padding': '10px', 'backgroundColor': '#ecf0f1', 'borderRadius': '5px'}),
